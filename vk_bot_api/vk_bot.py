...
def get_main_keyboard():
    ...
    keyboard.add_line()
    keyboard.add_button('️⚙️ Настроить параметры поиска',
...
def get_profiles_keyboard():
    ...
    keyboard.add_button('➡️ Следующий',
                        color=VkKeyboardColor.PRIMARY)
    keyboard.add_line()
    keyboard.add_button('📋 Информация о поиске',
                        color=VkKeyboardColor.SECONDARY)
...
def run_bot(adapter):
    ...
    def delete_candidates_on_parameter_change(user_id):
        ...
        try:
            # Находим всех кандидатов у которых favorite_status=0 и blacklist_status=0
            candidates_to_delete = adapter.session.query(Candidate).filter(
    ...
    def get_candidates_count_to_delete(user_id):
        ...
        count = adapter.session.query(Candidate).filter(
            Candidate.searcher_vk_id == user_id,
            Candidate.favorite_status == 0,
            Candidate.blacklist_status == 0
        ).count()
    ...
    def search_and_save_candidates(user_id, count=50):
        ...
        candidates = search_vk_users(ACCESS_TOKEN_VK, city_id, age, age,
                                     search_gender)
        ...
        for candidate in candidates[:count]:
            ...
            result = adapter.save_candidate_with_photos(candidate, photos,
                                                        user_id)
            if result:
                saved_count += 1
    ...
    def get_next_candidate(user_id):
        ...
        candidate = adapter.session.query(Candidate).filter(
            Candidate.searcher_vk_id == user_id,
        ...
        photos = adapter.session.query(Photo).filter(
            Photo.candidates_id == candidate.candidate_id
        ).all()
    ...
    def get_next_favorite(user_id):
        ...
        current_favorite = adapter.session.query(Candidate).filter(
            Candidate.searcher_vk_id == user_id,
    ...
    def get_next_blacklist(user_id):
        ...
        current_blacklist = adapter.session.query(Candidate).filter(
            Candidate.searcher_vk_id == user_id,
    ...
    def show_candidate(user_id, candidate_data):
        ...
        if not candidate_data:
            ...
            write_msg(user_id, "🔄 Ищу новых кандидатов...",
            ...
            if saved_count > 0:
                ...
                write_msg(user_id, "Кандидаты не найдены 😢",
                          get_main_keyboard())
                return
    ...
    def show_favorite(user_id, favorite_data):
        ...
        if attachments:
            write_msg(user_id, message, get_favorites_keyboard(),
                      ','.join(attachments))
        else:
            write_msg(user_id, message + "\n\n⚠️ Нет фото профиля",
    ...
    def show_blacklist(user_id, blacklist_data):
        ...
        if attachments:
            write_msg(user_id, message, get_blacklist_keyboard(),
                      ','.join(attachments))
        else:
            write_msg(user_id, message + "\n\n⚠️ Нет фото профиля",
    ...
    def show_current_settings(user_id):
        ...
        if user_data:
            ...
            write_msg(user_id, "У вас еще нет настроек поиска. "
    ...
    welcome_message = "👋 Я — бот для знакомств «Conspicere» - Взаимный взгляд.\n" \
                      "📋 Что я умею:\n" \
    ...
    while True:
        try:
            ...
            for event in events:
                if event.type == VkEventType.MESSAGE_NEW and event.to_me:
                    ...
                    if text.lower() in ['привет', 'старт', 'начать', 'start',
                                        '👋']:
                        write_msg(user_id, welcome_message,
                                  get_start_keyboard())
                    ...
                    # Смотреть анкеты
                    elif (text == '👀 Смотреть анкеты' or 'смотреть анкеты'
                          in text.lower()):
                        existing_user = adapter.get_user_data(user_id)

                        if not existing_user:
                            write_msg(user_id, "Сначала настройте"
                                               " параметры поиска!",
                                      get_start_keyboard())
                            continue
                    ...
                    # Кнопка "Нравится"
                        ...
                        current_candidate = adapter.session.query(
                            Candidate).filter(
                            Candidate.searcher_vk_id == user_id,
                            Candidate.view_status == 2
                        ).first()
                        ...
                        if not current_candidate:
                            write_msg(user_id, "Сначала выберите кандидата",
                                      get_main_keyboard())
                            continue

                        # Добавляем в фавориты: favorite_status=1, view_status=1
                        current_candidate.favorite_status = 1
                        current_candidate.view_status = 1
                        adapter.session.commit()

                        write_msg(user_id, "✅ Добавлено в избранное!",
                                  get_profiles_keyboard())
                    ...
                    # Кнопка "В черный список"
                    elif (text == '🚫 В черный список' or 'в черный список'
                          in text.lower()):
                        current_candidate = adapter.session.query(
                            Candidate).filter(
                            Candidate.searcher_vk_id == user_id,
                        ...
                        # Добавляем в черный список: blacklist_status=1, view_status=1
                        current_candidate.blacklist_status = 1
                        current_candidate.view_status = 1
                        adapter.session.commit()

                        write_msg(user_id, "✅ Добавлено в черный список!",
                                  get_profiles_keyboard())
                    ...
                    # Мои фавориты
                        ...
                        favorites_count = adapter.session.query(
                            Candidate).filter(
                            Candidate.searcher_vk_id == user_id,
                        ...
                        if favorites_count == 0:
                            write_msg(user_id,
                                      "❤️ В вашем избранном пока никого нет 😢",
                                      get_main_keyboard())
                            continue
                    ...
                    # Черный список
                        ...
                        blacklist_count = adapter.session.query(
                            Candidate).filter(
                            Candidate.searcher_vk_id == user_id,
                        ...
                        write_msg(user_id,
                                  f"🚫 В черном списке: {blacklist_count} человек\n\nВыберите действие:",
                                  get_blacklist_keyboard())
                    ...
                    # Кнопка "Настроить параметры поиска"
                        ...
                        existing_user = adapter.get_user_data(user_id)
                        if not existing_user:
                            write_msg(user_id, "Сначала зарегистрируйтесь!",
                                      get_start_keyboard())
                            continue
                    ...
                    # Регистрация: возраст
                    elif user_id in temp_user_data and 'age' not in \
                            temp_user_data[
                                user_id]:
                    ...
                    # Регистрация: пол и сохранение
                        ...
                        if text in ['1', '2']:
                            ...
                            write_msg(user_id,
                                      ...
                                      f"Пол: {'Мужской' if user_data['gender'] == 2 else 'Женский'}\n\n"
                    ...
                    # Редактирование параметров: выбор параметра
                        ...
                        if text == '1' or 'возраст' in text.lower():
                            ...
                            write_msg(user_id, "Изменения отменены.",
                                      get_main_keyboard())
                        else:
                            write_msg(user_id, "Выберите параметр для "
                                               "изменения (1-4):")
                    ...
                    # Редактирование: пол
                        ...
                        if text in ['1', '2']:
                            ...
                            write_msg(user_id,
                                      f"✅ Пол обновлен на {'женский' if text == '1' else 'мужской'}!\n\n"
                                      f"Новые параметры поиска:\n"
                                      f"• Возраст: {user_data.get('age', 'не указан')} лет\n"
                    ...
                    # Кнопка "Удалить фаворита"
                        ...
                        current_favorite = adapter.session.query(
                            Candidate).filter(
                            Candidate.searcher_vk_id == user_id,
                            Candidate.favorite_status == 2
                        ).first()
                        ...
                        if not current_favorite:
                            write_msg(user_id, "Сначала выберите фаворита",
                                      get_favorites_keyboard())
                            continue

                        # Удаляем фото
                        adapter.session.query(Photo).filter(
                            Photo.candidates_id == current_favorite.candidate_id
                        ).delete()
                        ...
                        write_msg(user_id, "🗑️ Фаворит удален!",
                                  get_favorites_keyboard())
                    ...
                    # Кнопка "Удалить из ЧС"
                        ...
                        current_blacklist = adapter.session.query(
                            Candidate).filter(
                            Candidate.searcher_vk_id == user_id,
                            Candidate.blacklist_status == 2
                        ).first()
                        ...
                        if not current_blacklist:
                            write_msg(user_id, "Сначала выберите "
                                               "кандидата из ЧС",
                                      get_blacklist_keyboard())
                            continue
                        ...
                        # Удаляем фото
                        adapter.session.query(Photo).filter(
                            Photo.candidates_id == current_blacklist.candidate_id
                        ).delete()
                        ...
                        write_msg(user_id, "🗑️ Удалено из черного списка!",
                                  get_blacklist_keyboard())
                    ...
                    # Информация о поиске
                        ...
                        existing_user = adapter.get_user_data(user_id)
                        if existing_user:
                            unviewed_count = adapter.session.query(
                                Candidate).filter(
                                Candidate.searcher_vk_id == user_id,
                                Candidate.view_status == 0,
                                Candidate.favorite_status == 0,
                            ...
                            message = (
                                ...
                                f"📊 Параметры поиска:\n"
                                f"• Возраст: {existing_user.get('age', 'не указан')} лет\n"
                                f"• Пол: {'Мужской' if existing_user.get('gender') == 2 else 'Женский'}\n"
                                f"• Город: {existing_user.get('city', 'не указан')}"
                            )
                            write_msg(user_id, message,
                                      get_profiles_keyboard())
                        else:
                    ...
                    # Неизвестная команда
                        ...
                        if user_id in temp_user_data or user_id in edit_user_data:
                            write_msg(user_id,
                                      "Завершите регистрацию/редактирование или введите 'отмена'")
                        else:
                            write_msg(user_id, "Не понял. Нажмите "
                                               "'Начать поиск'",
                                      get_start_keyboard())
            ...
            print(
                f"Неожиданная ошибка: {e}. Продолжение работы через 5 секунд...")
            import traceback
            traceback.print_exc()
            time.sleep(5)
